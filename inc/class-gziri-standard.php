...
function gziri_standard_init() {
	if (!class_exists('WC_Gziri_Standard')) {
		class WC_Gziri_Standard extends WC_Shipping_Method {
			...
			public function __construct($instance_id = 0) {
				$this->id = 'gziri_standard';
				$this->instance_id  = absint($instance_id);
				$this->method_title = __('GZIRI Standard', 'gziri');				
				$this->method_description = __('GZIRI Standard package shipping method. Estimated delivery in Tbilisi is next business day if ordered before 19:00. In regions - within 7 business days.', 'gziri');
				$this->supports = array(
					'shipping-zones',
					'instance-settings',
			...
			public function init_form_fields() {
                $this->instance_form_fields = array(
                    'title' => array(
                        ...
                        'default'     => __('GZIRI Standard', 'gziri'),
                        'desc_tip'    => true,
                    ),
					'tax_status' => array(
			...
			public function calculate_shipping($package = array()) {
				...
				if ($country == GZIRI_REQUIRED_COUNTRY && !empty($city)) {
					...
					$log = '[' . date('Y-m-d H:i:s') . ']: Country is not Georgia or city is not set' . PHP_EOL;
					file_put_contents(plugin_dir_path(__FILE__) . 'logs.log', $log, FILE_APPEND);
				}
...
add_action('woocommerce_shipping_init', 'gziri_standard_init');

function add_gziri_standard($methods) {
	$methods['gziri_standard'] = 'WC_Gziri_Standard';
	return $methods;
}
add_filter('woocommerce_shipping_methods', 'add_gziri_standard');
...
function gziri_fast_init() {
	if (!class_exists('WC_Gziri_Fast')) {
		class WC_Gziri_Fast extends WC_Shipping_Method {
			...
			public function __construct($instance_id = 0) {
				...
				$this->method_title = __('GZIRI Fast', 'gziri');				
				$this->method_description = __('GZIRI Fast package shipping method. Estimated delivery in Tbilisi is same business day if ordered before 15:00. This method is not available in regions.', 'gziri');
				$this->supports = array(
					'shipping-zones',
					'instance-settings',
...
add_action('woocommerce_shipping_init', 'gziri_fast_init');

function add_gziri_fast($methods) {
	$methods['gziri_fast'] = 'WC_Gziri_Fast';
	return $methods;
}
add_filter('woocommerce_shipping_methods', 'add_gziri_fast');